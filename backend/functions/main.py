    ...
    try:
        ...
        if '/tle/' in path:
            ...
            if path.endswith('/list'):
                ...
                orbit_type = path.rstrip('/').split('/')[-1]
                if orbit_type and orbit_type not in ('orbits', 'api'):
                    return orbit_info_path_endpoint(orbit_type)

        elif '/animation/' in path:
            if path.endswith('/orbit-frames'):
                return animation_orbit_frames_endpoint(request)

        elif '/perturbations/' in path:
            if path.endswith('/advanced'):
                return advanced_perturbations_endpoint(request)

        elif '/mission-planning/' in path:
            if path.endswith('/launch-windows'):
                return mission_launch_windows_endpoint(request)
        ...
        return (
            jsonify({
                ...
                'available_endpoints': [
                    ...
                    '/api/orbits/list',
                    '/api/orbits/info',
                    '/api/orbits/<orbit_type>',
                    '/api/ground-track/calculate',
                    '/api/ground-track/access',
                    '/api/animation/orbit-frames',
                    '/api/perturbations/advanced',
                    '/api/mission-planning/launch-windows'
                ]
            }),
            404,
            cors_headers()
...
def orbit_info_path_endpoint(orbit_type: str):
    """Get information about specific orbit type from path parameter."""
    try:
        orbit_info = get_orbit_info(orbit_type)
        response = jsonify(orbit_info)
        response.headers.update(cors_headers())
        return response
    except Exception as e:
...
def animation_orbit_frames_endpoint(request: Request):
    """Generate orbit animation frames for frontend playback."""
    data = request.get_json()
    try:
        from animation import OrbitAnimationGenerator
        ...
        frames = [frame for frame in generator.generate_frames()]
...
# ============================================================================
# ADVANCED PERTURBATION ENDPOINTS
# ============================================================================

def advanced_perturbations_endpoint(request: Request):
    """Propagate with advanced perturbations (J2, J3, J4, drag)."""
    data = request.get_json()
    try:
        ...
        response = jsonify({
            ...
            'final_elements': {
                ...
                'inclination_deg': float(result['i'] * RAD_TO_DEG),
                'raan_deg': float(result['omega_cap'] * RAD_TO_DEG),
                'arg_perigee_deg': float(result['omega'] * RAD_TO_DEG),
                'true_anomaly_deg': float(result['nu'] * RAD_TO_DEG)
            },
...
def mission_launch_windows_endpoint(request: Request):
    ...
    try:
        ...
        response = jsonify({
            ...
            'launch_site_longitude_deg': data.get('launch_site_longitude_deg'),
